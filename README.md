...
- A unified user data object `AuthenticatedUser` via the `me()` method
...
- **ClientCredentials**: indicates whether the provider supports the Client Credentials Grant (implements the `ClientCredentials` interface)
- **TokenInvalidate**: indicates whether the provider is capable of revoking/invalidating an access token (implements the `TokenInvalidate` interface)
- **TokenRefresh**: indicates whether the provider is capable of refreshing an access token (implements the `TokenRefresh` interface)
